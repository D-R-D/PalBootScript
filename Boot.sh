...
## サーバー設定処理
default_setting_path="/root/Steam/steamapps/common/PalServer/DefaultPalWorldSettings.ini"
setting_path="/root/Steam/steamapps/common/PalServer/Pal/Saved/Config/LinuxServer/PalWorldSettings.ini"
option_settings_line=$(grep 'OptionSettings=' "$setting_path")

if [ "$option_settings_line" = "" ]; then
    echo "エラー: コンフィファイルが未生成です。"
    echo "./DefaultPalWorldSettings.iniの内容を./Pal/Saved/Config/LinuxServer/PalWorldSettings.iniにコピーします。"
    while [ ! -e "${default_setting_path}" ]; do
        sleep 1  # 適切な待機時間を指定してください。この例では5秒に設定しています。
    done
    option_settings_line=$(grep 'OptionSettings=' "$default_setting_path")
fi
# 各設定の正規表現パターン
